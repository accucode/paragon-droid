...
public class WheelRecycle
{
    ...
    {
        return getCachedView(_emptyItems);
    }
    ...
    {
        int count = _wheel.getViewAdapter().getItemsCount();

        if ( (index < 0 || index >= count) && !_wheel.isCyclic() )
            // empty view
            _emptyItems = addView(view, _emptyItems);
        else
        {
            while ( index < 0 )
                index = count + index;
            index %= count;
            _items = addView(view, _items);
        }
    }
